...
\author{An experimental technical note}
\date{July 12, 2026}
...
construction in Yasuda's algorithm:
\[
...
intended as a new theorem or a formal verification of the algorithm. It records a
reproducible computational experiment, including selected Macaulay2 input and output.
The code was developed with the assistance of an AI system; the mathematical setup,
examples, expected structures, and checks were selected and reviewed by the author.
\end{abstract}
...
The computations were run with Macaulay2 1.24.11 on macOS. The relevant bundled packages
include \texttt{Truncations}, \texttt{MinimalPrimes}, and \texttt{Saturation}. From the
project root, the standard suite is run by:
...
so the test supplies $r=(2,0)$ and records that this bound is experimental.
