...
\begin{verbatim}
data  = steinHomData(S, graphIdeal)
...
\texttt{graphIdeal} is the bihomogeneous ideal \(I_{\Gamma_f}\subset S\)
cutting the graph out of that product, so that \(R=S/I_{\Gamma_f}\). The
map \(f\) is never passed as such: it enters only through its graph.
...
The returned \texttt{data} carries everything the later steps need: the
bound \(\mathbf{r}\) and whether it is certified, the ring \(R\), the
truncation \(R_{\geq\mathbf{r}}\), the module
\(\Hom_R(R_{\geq\mathbf{r}},R)_{\geq0}\) together with the matrix of its
...
A homomorphism \(\varphi\colon R_{\geq\mathbf{r}}\to R\) is not itself an
element of \(R\), so to see the strand as a ring one has to evaluate.
Fix a nonzero \(\gamma\in R_{\geq\mathbf{r}}\) and send \(\varphi\) to
\(\varphi(\gamma)/\gamma\), a quotient formed in the localization
\(R[1/\gamma]\); this is Lemma 5.2. \texttt{gammaIndex} is which
generator of \(R_{\geq\mathbf{r}}\) to use as that \(\gamma\); the
examples all take \texttt{0}.

\texttt{baseImages} lists elements of \(R\) generating the subring
\(A=R_{(0,\geq0)}\), over which the strand is a finite module. Since
\(R\) is generated in the two block degrees, \(A\) is exactly the subring
generated by the target-block variables, so in every example here
\texttt{baseImages} is the list of coordinates of \(X\). The point of
the whole computation is that the strand \(C\) is larger than \(A\): the
extra algebra generators are the evaluated Hom generators, and they are
what distinguishes \(Z\) from \(X\).

The returned \texttt{cData} holds the graded coordinate algebra of \(Z\)
under \texttt{"ring"} and its relations under \texttt{"definingIdeal"},
presented on the chosen generators --- first the \texttt{baseImages},
then one variable per evaluated Hom generator. It also keeps \(A\), the
strand as a finite \(A\)-module with its presentation, and the
localization and \(\gamma\) that were used.

\subsubsection*{The graph ---
  \texorpdfstring{{\normalfont\ttfamily directSteinGraph}}{directSteinGraph}}

At this point the generators of \(Z\) are available as actual functions
on \(\Gamma_f\cong Y\), so the graph of \(h\colon Y\to Z\) can be formed
directly: it is the kernel of the ring map sending each variable of
\(Z\) to the corresponding function, computed in \(R[1/\gamma]\), with a
Rees parameter carrying the projective scaling of \(Z\). Working in the
localization is what saturates the result. The returned \texttt{gData}
holds that ideal under \texttt{"graphIdeal"}, in a ring whose variables
are those of \texttt{S} together with the algebra generators of \(Z\).
