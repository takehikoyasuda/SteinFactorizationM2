...
\texttt{gammaIndex} is an integer rather than a ring element: it is the
position, counted from \texttt{0}, of \(\gamma\) in the list of generators
of \(R_{\geq\mathbf{r}}\). By the independence just noted the subring
obtained does not depend on it; only the localization the computation is
carried out in does. The examples all take \texttt{0}, the first
generator.
...
first the \texttt{baseImages}, then one variable for each evaluated Hom
...
Give \(Z\) a homogeneous coordinate \(z_i\) for each \(c_i\), of bidegree
\((0,b_i)\), and form
\[J=k[\,\text{variables of }S,\ z_0,\dots,z_N\,]\longrightarrow
R[1/\gamma][t],\qquad
x\longmapsto \bar x,\quad z_i\longmapsto c_i\,t^{b_i}.\]
Each variable \(x\) of \(S\) goes to its class \(\bar x\) in
\(R=S/I_{\Gamma_f}\), taken inside \(R[1/\gamma]\); that is all the first
half of the map does. The graph of \(h\colon Y\to Z\) is the kernel, a
...
Writing \(F=\sum_\alpha F_\alpha z^\alpha\) with \(F_\alpha\) in the
variables of \(S\),
\[F\longmapsto\sum_\alpha
F_\alpha\,c^\alpha\,t^{\langle b,\alpha\rangle},\]
and since \(t\) is a free variable this vanishes if and only if
\(\sum F_\alpha c^\alpha=0\) separately for each value of
