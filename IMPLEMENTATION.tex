...
Those two arguments are all that is needed. Corollary 4.3 also uses the
dimensions \(d_s\) of the ambient projective spaces and the sums \(c_s\) of
the variable degrees in each block, but the grading of \texttt{S} determines
them.  The function \texttt{blockDegreeData S} returns the four values; in a
weighted block \(c_s\) is the weight sum rather than the variable count.
...
By the independence just noted, the subring obtained does not depend on the
chosen \(\gamma\).  The one-argument call therefore uses the first generator
of \(R_{\geq\mathbf r}\).  The two-argument form
\texttt{steinCoordinateAlgebra(homData,i)} remains available when a particular
localization is wanted; \texttt{i} is the generator position counted from
\texttt{0}.
...
Let \(y_0,\dots,y_n\) be the variables in the source block of \(S\), of
degrees \((d_j,0)\). Each \(c_i\) becomes a homogeneous coordinate \(z_i\)
on \(Z\), of degree \((0,b_i)\). The map to take the kernel of is
\[\Psi\colon k[y_0,\dots,y_n,z_0,\dots,z_N]\longrightarrow R[1/\gamma],\qquad
\Psi(y_j)=\bar y_j,\qquad \Psi(z_i)=\iota(c_i).\]
The graph of \(h\colon Y\to Z\) is \(\ker\Psi\). The original target
coordinates remain available inside \(R[1/\gamma]\), but they do not occur
in the source of \(\Psi\): putting them beside the \(z_i\) in the second
grading block would collapse two independent projective scalings and leave
an extra parameter. Since \(\iota\) preserves the second degree, \(\Psi\) is
bigraded without an auxiliary Rees parameter.
...
The timings and transcripts below were recorded with Macaulay2 1.24.11 on
macOS (Apple Silicon, 2023).  Current development and testing use Macaulay2
1.26.06; the timings have not been remeasured.  Version 1.24.11 is the oldest
version the package is believed to support.  The relevant bundled packages include
...
the completed examples. In the identity, square, cube, and
positive-dimensional-fiber tests, elimination of either coordinate block
recovers the expected source or Stein intermediate, and substituting the
finite map \(Z\to X\) into the input graph verifies \(g\circ h=f\). For the
cubic map, the three localization elements \(y_1^2,y_0y_1,y_0^2\) give the
same coordinate and graph ideals after identifying the automatically named
variables. The computed Hilbert functions, exceptional
