...
intermediate \(Z\) with respect to \(g^*\sO_X(1)\). That polarization is
pulled back from \(X\), so it need not be \(\sO_Z(1)\): in every example
below the finite map \(g\) is a coordinatewise power map, and
\(g^*\sO_X(1)=\sO_Z(d)\) for some \(d>1\). What the strand computes is
then not the full homogeneous coordinate ring of \(Z\), but the subring
of it spanned by the graded pieces whose degree is divisible by \(d\) ---
for \(Z=\PP^m\), the span of the monomials of degree \(d,2d,3d,\dots\).
That subring is classically called the \(d\)-th Veronese subring, since
it is the homogeneous coordinate ring of \(Z\) in its \(d\)-uple Veronese
embedding; the sections below refer to it by that name.
...
the degrees of those variables, as in Proposition 4.2. In the standard
bigraded case \(c_s\) is simply the number of variables in the block;
under a weighted grading it is the larger weight sum. All four numbers
are determined by the degrees of the ambient ring, so they can be left
off --- \texttt{steinHomDataFromRing(S, graphIdeal)} reads them off ---
and when they are supplied they are checked against the ring rather than
trusted.

What the construction requires is only that the variables split into a
source block of degrees \((\ast,0)\) and a target block of degrees
\((0,\ast)\): that split is what lets the first degree see the source
direction alone, so that the \((0,\geq0)\)-strand means what it does.
The degrees within a block need not be \(1\), and weighted projective
spaces are allowed, as in the paper. The examples of Section
\ref{sec:examples} are all standard bigraded, but
\texttt{tests/weighted.m2} runs the same construction over \(\PP(1,2)\).
The first function computes the truncated Hom module and records the bound.
...
count would give \(2\). Passing \(2\) by hand does not fail loudly, since
Corollary 4.3 subtracts \(c_1\) and too small a value only enlarges the
bound --- the answer stays correct and the computation merely does more
work. That silence is why the four numbers are now read off the ring
rather than trusted.
