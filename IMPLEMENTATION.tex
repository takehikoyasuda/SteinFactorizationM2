...
\texttt{evaluationElementIndex} is an integer rather than a ring element:
it is the position, counted from \texttt{0}, of \(\gamma\) in the list of
generators of \(R_{\geq\mathbf{r}}\). By the independence just noted the
subring obtained does not depend on it; only the localization the
computation is carried out in does. The examples all take \texttt{0}, the
first generator.
