...
in which $h$ is proper with $h_*𝒪_Y=𝒪_Z$ (so that $h$ has connected fibers) and $g$ is finite; concretely $Z=\mathrm{Spec}_X f_*𝒪_Y$. The purpose of this project is to turn an algorithm described by Yasuda (2026) into an executable Macaulay2 prototype and to see how it behaves on concrete examples from projective geometry.
...
$$H^0(Y,f^*𝒪_X(n))=H^0(X,f_*𝒪_Y\otimes𝒪_X(n))=H^0(Z,g^*𝒪_X(n)).$$
That is, the $(0,\geq0)$-strand is the section ring of the Stein intermediate $Z$ with respect to $g^*𝒪_X(1)$. This is why the examples below produce Veronese section rings rather than the usual homogeneous coordinate rings: the polarization on $Z$ is pulled back from $X$.
...
Thus the Stein intermediate is the source $ℙ^1$, embedded as the conic $X_0X_1-z^2=0$: since $f$ is finite, $Z\cong Y=ℙ^1$, polarized by $f^*𝒪(1)=𝒪(2)$, whose section ring is the second Veronese ring of $ℙ^1$.
...
Each of the four fiber components is a copy of $ℙ^1$. The computed Stein intermediate is the degree-two Veronese model of $ℙ^2$, and the connected part is the projection to $ℙ^2$.

Here $Z=ℙ^2$ carries the polarization $g^*𝒪(1)=𝒪(2)$, so its section ring is the second Veronese ring of $ℙ^2$, i.e. the subring of $k[x_0,x_1,x_2]$ generated by the degree-2 monomials. Its Hilbert function is $H(1)=6$ (the 6 monomials of degree 2 in 3 variables) and $H(2)=15$ (the 15 monomials of degree 4 in 3 variables). The computation reproduces these known invariants, which is what one expects if the algorithm identifies the Stein intermediate correctly.

### The blow-up of a line in $ℙ^3$

We next take the blow-down $\mathrm{Bl}_L(ℙ^3)\toℙ^3$ along a line $L$, followed by the coordinatewise square map on $ℙ^3$. The main numerical checks are:
...
The final example is the blow-up of $ℙ^3$ along the twisted cubic, followed by the coordinatewise square map. It is deliberately retained as a heavier benchmark. Here the source sits in $ℙ^{11}$, so the automatic Corollary 4.3 bound would require a minimal free resolution out to homological degree $11+3+1$; that computation did not finish within about ten minutes on the machine used here and was aborted. The test therefore supplies $r=(2,0)$ and records that this bound is experimental; with the supplied bound the example runs in about 11 seconds.
...
The resulting ring again has the Hilbert values of the second Veronese of $ℙ^3$. One degree-two module generator is algebraically redundant, so automatic graph construction is not yet completed for this example. This is a useful illustration of the difference between a bound that happens to work and a bound certified by Corollary 4.3.
...
|---------|-------|---------|----------------------|-------|
| $ℙ^1$ quadratic map | $(1,0)$ | 2 | $H(1)=3$ | prime |
| $ℙ^1$ cubic map | $(2,0)$ | 2 | twisted cubic | prime |
| $ℙ^1\timesℙ^2$ | $(2,0)$ | 3 | $H(1)=6,H(2)=15$ | prime |
| $\mathrm{Bl}_L(ℙ^3)$ | $(2,0)$ | 4 | $H(1)=10,H(2)=35$ | prime |
