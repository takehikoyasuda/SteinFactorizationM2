...
Node
  ...
  Inputs
    ...
    gammaIndex:ZZ
      ...
      $R_{\ge\mathbf{r}}$
    baseImages:List
      elements of $R$ generating the subring $A=R_{(0,\ge0)}$, over which the
      strand is a finite module
  Outputs
    :HashTable
      holding the graded coordinate algebra of $Z$ under {\tt "ring"} and its
      relations under {\tt "definingIdeal"}, the same $C$ a second time as a
      finite $A$-module with its presentation, and the localization
      $R[1/\gamma]$ together with the $\gamma$ that defines it
  Description
    Text
      The strand $C$ consists of homomorphisms rather than of elements of $R$,
      so it carries no evident multiplication.  It acquires one by being
      realized inside a localization, $\varphi\mapsto\varphi(\gamma)/\gamma$;
      see @TO evaluateSteinGenerators@, where that evaluation is described.
      This function carries it out and then eliminates, returning $C$ as a
      quotient of a polynomial ring.

      Since $R$ is generated in the two block degrees, $A$ is exactly the
      subring generated by the target-block variables, so {\tt baseImages} is
      the list of coordinates of $X$ in every example here.  The point of the
      whole computation is that $C$ is larger than $A$: the extra algebra
      generators are the evaluated Hom generators, and they are what
    ...
    Example
      ...
      Igraph = ideal(y0^2*x1 - y1^2*x0);
      homData = steinHomData(S, Igraph);
      algebraData = steinCoordinateAlgebra(homData, 0, {x0,x1});
      algebraData#"definingIdeal"
    Text
      The polynomial ring is generated automatically, so its variables print as
      $p_0,p_1,\dots$; they come in a fixed order, first the {\tt baseImages}
      ...
      its own.)  So $p_0,p_1$ are the coordinates $x_0,x_1$ of $X$ and $p_2$ is
      the single evaluated Hom generator, and the relation reads
      $x_0x_1=p_2^2$: the Stein intermediate is $\mathbb{P}^1$ embedded as a
      conic, which is what a finite map $Y\to X$ should give, $Z\cong Y$
      polarized by $f^*\mathcal{O}(1)=\mathcal{O}(2)$.
    Text
    ...
    Example
      degrees algebraData#"strandAsAModule"
    Text
      By the independence of the evaluation from the choice of $\gamma$, the
      subring obtained does not depend on {\tt gammaIndex}; only the
...
Node
  ...
  Usage
    stab = steinDataByStabilization(S, Igraph, startBound, maxSteps, requiredMatches, baseImages, hilbertMax)
  Inputs
    S:Ring
      the ambient bigraded polynomial ring
    Igraph:Ideal
    ...
    baseImages:List
      generators of $A=R_{(0,\ge0)}$, passed on to
      @TO steinCoordinateAlgebra@
    hilbertMax:ZZ
...
Node
  ...
  Usage
    evals = evaluateSteinGenerators(homData, gammaIndex)
  Inputs
    homData:HashTable
      as returned by @TO steinHomData@ or @TO steinHomDataAtBound@
    gammaIndex:ZZ
